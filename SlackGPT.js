...
const azureOpenaiEndpoint = PropertiesService.getScriptProperties().getProperty("AZURE_OPENAI_ENDPOINT");
const azureOpenaiKey = PropertiesService.getScriptProperties().getProperty("AZURE_OPENAI_KEY");
const azureApiVersion = PropertiesService.getScriptProperties().getProperty("AZURE_API_VERSION");
const azureDeploymentName = PropertiesService.getScriptProperties().getProperty("AZURE_DEPLOYMENT_NAME");
// OpenAIのエンドポイントを設定する
const ENDPOINT = azureOpenaiEndpoint + "/openai/deployments/" + azureDeploymentName + "/chat/completions?api-version=" + azureApiVersion;
...
const fetchAIAnswerText = (tiggerMsg) => {
  ...
  // リクエストボディを作成する
  const requestBody = {
    messages: [{ "role": "user", "content": character }, ...msgsForChatGpt],
  };
